...
function openWebSocket() {
  const videoElement = document.getElementById("videoElement");
  const videoElement2 = document.getElementById("videoElement2"); // Thêm videoElement2
  ...
  websocket.onopen = async () => {
    if (device) {
      await Promise.all([
        getVideoStream({ deviceId: device.id }).then(async (stream) => {
          videoElement.srcObject = stream;
        }),
        getWebcamStream().then(async (stream) => { // Sử dụng webcam của máy tính cho Camera 2
...
startWebcamButton.addEventListener("click", () => {
  getWebcamStream().then((stream) => {
    ...
    console.log("Webcam stream is now available for Camera 2.");
  });
});

// Hàm để lấy luồng video từ webcam
...
// Hàm để dừng luồng video từ webcam
function stopWebcamStream() {
  if (videoElement2.srcObject) {
    const stream = videoElement2.srcObject;
